...
export default function PlayerControls() {
  const [{ token, playerState }, dispatch] = useStateProvider();
    const changeTrack =async (type) => {
      await axios.post(
          ...
          dispatch({ type: reducerCases.SET_PLAYLING, currentlyPlaying:null });
    }
    const changeState = async () => {
        const state = playerState ? "pause" : "play";
  ...
  return (
    <Container>
      ...
      <div className="state">
        {playerState ? <BsPauseCircleFill onClick={changeState} /> : <BsFillPlayCircleFill onClick={changeState} />}
      </div>
